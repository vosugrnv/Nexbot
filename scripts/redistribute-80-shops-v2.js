...
fs.writeFileSync(
  ...
  "/* 80 gian hàng chuẩn Vua MMO — token riêng từng shop */\n" +
    ...
    "function shopHref(shop){ if(!shop) return '/shop'; const slug=shop.slug||String(shop.name||'').toLowerCase().replace(/[^a-z0-9]+/g,'-'); return slug?('/'+slug):'/shop'; }\n" +
...
</head><body><p><a href="/shop?token=${encodeURIComponent(s.token)}">${s.name}</a></p></body></html>`);
}
